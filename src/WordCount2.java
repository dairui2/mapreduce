...
//                hasMoreTokens() 方法是用来测试是否有此标记生成器的字符串可用更多的标记。
    ...
    public static void main(String[] args) throws Exception {
        ...
        //删除已经存在的输出目录
        Path mypath = new Path("hdfs://Centpy:9000/test/wordcount-out");//输出路径
        ...
        job.setJarByClass(WordCount2.class);//我们在hadoop集群上运行作业的时候，要把代码打包成一个jar文件，然后把这个文件
...
//                                这句话要好好理解!!!
        ...
        FileInputFormat.addInputPath(job, new Path(
                "hdfs://Centpy:9000/test/word.txt"));//FileInputFormat.addInputPath（）指定的这个路径可以是单个文件、一个目录或符合特定文件模式的一系列文件。
        //从方法名称可以看出，可以通过多次调用这个方法来实现多路径的输入。        
        FileOutputFormat.setOutputPath(job, new Path(
                "hdfs://Centpy:9000/hdfsOutput"));//只能有一个输出路径，该路径指定的就是reduce函数输出文件的写入目录。
